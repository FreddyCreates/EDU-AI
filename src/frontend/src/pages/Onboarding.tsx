...
export default function Onboarding() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Welcome, ${name.trim()}! Let's start learning!`);
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none fixed inset-0">
        <div className="absolute top-1/4 left-1/3 h-96 w-96 rounded-full bg-[rgba(0,210,255,0.04)] blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 h-64 w-64 rounded-full bg-[rgba(160,100,255,0.04)] blur-3xl" />
      </div>
      ...
      <div className="w-full max-w-lg relative z-10">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 glass-portal-student rounded-full px-4 py-1.5 mb-5">
            <span className="h-1.5 w-1.5 rounded-full bg-[oklch(0.78_0.22_200)] animate-pulse" />
            <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-[oklch(0.78_0.22_200)]">
              STUDENT OS · ONBOARDING
            </span>
          </div>
          <h1 className="font-display text-3xl md:text-4xl font-black text-foreground leading-tight">
            Welcome to{" "}
            <span className="text-[oklch(0.78_0.22_200)]">EduAI</span>
          </h1>
          <p className="mt-2 text-foreground/50 text-sm">
            Your personal sovereign AI study companion — K through 12, forever.
          </p>
        </motion.div>
                  ...
                  <div key={s} className="flex items-center gap-2">
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-smooth ${
                        step >= s
                          ? "bg-[oklch(0.78_0.22_200)] text-[oklch(0.07_0.01_260)] shadow-[0_0_12px_rgba(0,210,255,0.4)]"
                          : "bg-[rgba(255,255,255,0.06)] text-foreground/30"
                      }`}
                    >
                      {step > s ? "✓" : s}
                    </div>
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  <div className="text-center space-y-6">
                    ...
                    <p className="text-foreground/50 text-sm max-w-md mx-auto leading-relaxed">
                      A sovereign learning companion built to grow with you from
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-foreground/40 mt-1">
                      This is how your AI guides will greet you.
                    </p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-foreground/25 text-right font-mono">
                      {name.length}/50
                    </p>
                  </div>
                  <Button
              ...
              {step === 2 && (
                ...
                >
                  <div>
                    <h2 className="font-display text-xl font-bold text-foreground">
                      Hi{name ? `, ${name.trim()}` : ""}! What grade are you in?
                    </h2>
                    <p className="text-sm text-foreground/40 mt-1">
                      We’ll load the right curriculum just for you.
                    </p>
                  </div>
                  ...
                  >
                    {GRADE_LEVELS.map((grade) => (
                      <button
                        ...
                        className={`flex flex-col items-center gap-1 p-2.5 rounded-2xl border text-center transition-glass cursor-pointer ${
                          gradeLevel === grade.value
                            ? "glass-portal-student border-[rgba(0,210,255,0.4)] shadow-[0_0_12px_rgba(0,210,255,0.2)]"
                            : "glass-sm border-[rgba(255,255,255,0.06)] hover:border-[rgba(0,210,255,0.2)] hover:bg-[rgba(0,210,255,0.04)]"
                        }`}
                      >
                  ...
                  {gradeLevel && (
                    ...
                    >
                      <span className="text-base">
                        {
                          GRADE_LEVELS.find((g) => g.value === gradeLevel)
                            ?.emoji
                        }
                      </span>
                      <span className="text-sm font-semibold text-[oklch(0.78_0.22_200)]">
                        {
                          GRADE_LEVELS.find((g) => g.value === gradeLevel)
                            ?.label
                        }{" "}
                    ...
                    >
                      {createProfile.isPending ? (
                        <span className="flex items-center gap-2">
                          <span className="w-4 h-4 border-2 border-[oklch(0.07_0.01_260)]/40 border-t-[oklch(0.07_0.01_260)] rounded-full animate-spin" />
                          Setting up…
                        </span>
                      ) : (
                        "🚀 Start Learning"
        ...
        >
          🔒 Sovereign · Private · Yours forever · No account required to start
        </motion.p>
      </div>
    </div>
  );
